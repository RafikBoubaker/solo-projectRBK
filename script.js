...
document.addEventListener('DOMContentLoaded', function (event) {
  ...
  document.querySelector('.record').textContent = 'Highest Record: ' + localStorage.getItem(".record");
  console.log(secretNumber);
});


document.querySelector('.check-btn').addEventListener('click', function () {
  var inputNumber = document.querySelector('.guess-number').value;
...
document.querySelector('.again-btn').addEventListener('click', function () {
  ...
  document.querySelector('.message').textContent = 'Guess the number!';
  document.querySelector('.number').textContent = '?';
  document.querySelector('body').style.backgroundColor = 'rgba(231, 24, 24, 0.9)';
});
